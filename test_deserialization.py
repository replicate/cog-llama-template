...
def test_in_memory_cli_deserialization():
    ...
    if result.returncode != 0:
        raise Exception(
            f"gcloud storage cp command failed with return code {result.returncode}: {result.stderr.decode('utf-8')}"
        )
...
def download_blob_to_stream(bucket_name: str, source_blob_name: str, n: int = 4):
    ...
    def _partition_file(size: int, bucket: str, blob: str, n: int) -> List[dict]:
        ...
        for i in range(n):
            ...
            partitions.append(
                {"start": start, "end": end, "bucket": bucket, "blob": blob}
            )
            start = end + 1
            end += split
...
def test_python_deserialization():
    ...
    obj = download_blob_to_stream(
        bucket_name=bucket_name, source_blob_name=source_name, n=24
    )
