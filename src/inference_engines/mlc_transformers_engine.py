...
class MLCTransformersEngine(Engine):
    """
    MLC for base models, transformers for fine-tunes.
    """

    def __init__(self, mlc_args: dict, transformers_args: dict) -> None:
        ...
        if os.getenv("COG_WEIGHTS") or os.getenv("REPLICATE_HOTSWAP") == "1":
            self.engine = TransformersEngine(**transformers_args)
        else:
            # can't run if MLC is imported
    ...
    def load_lora(self, lora_data: dict) -> Any:
        ...
        """
        if not isinstance(self.engine, TransformersEngine):
    ...
    def __call__(
        ...
        **kwargs,
        ...
        print(f"MLC: {not isinstance(self.engine, TransformersEngine)}")
        gen = self.engine(
            prompt,
