...
from tests.test_utils import get_image_name, process_log_line, capture_output, wait_for_server_to_be_ready
...
def server():
    ...
    volume_mount = f"{current_directory}/training_output.zip:/src/local_weights.zip"
...
def test_input_too_long(server):
    ...
    data = {
        "input": {
            "prompt": " a" * 6000,  # Assuming this string will produce more than 4096 tokens.
            "max_new_tokens": 25,
            # Add other parameters here
        }
    }
    ...
    assert response_data['error'].startswith(error_msg_prefix), f"Expected the error message to start with '{error_msg_prefix}'"
