...
BOS = "<s>"
# note - tokenizer adds bos, don't want it twice. 
PROMPT_TEMPLATE = f"{B_INST} {{instruction}} {E_INST}"

def prompt_preprocessor(prompt):
    # assumes 
    prompt = prompt.strip("\n").removeprefix(BOS + B_INST).removesuffix(E_INST).strip()
...
REMOTE_DEFAULT_INFERENCE_FILES_TO_DOWNLOAD = REMOTE_TRAINING_FILES_TO_DOWNLOAD
